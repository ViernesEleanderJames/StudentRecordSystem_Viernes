...
namespace StudentRecordSystem_Viernes
{
    public partial class StudentPage_Individual : Form
    {
        ...
        {
            MySqlConnection connection = new MySqlConnection("server=localhost;database=StudentInfoDB;user=root;password=12345;");
            try
            {
                connection.Open();
                string query = "SELECT s.studentId, s.firstName, s.lastName, s.middleName, " +
                               "s.houseNo, s.brgyName, s.municipality, s.province, s.region, s.country, " +
                               "s.birthdate, s.age, s.studContactNo, s.emailAddress, s.guardianFirstName, " +
                               "s.guardianLastName, s.hobbies, s.nickname, c.courseName, y.yearLvl " +
